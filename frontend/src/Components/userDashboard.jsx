...
const Dashboard = () => {
    ...
    axios.get('http://localhost:5000/user-info', {
      ...
      .then(response => {
        ...
        if (response.data._id) {
          axios.get('http://localhost:5000/service-requests/user', {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
          })
  ...
  const handleDelete = () => {
    ...
    if (confirmDelete) {
      axios.delete(`http://localhost:5000/delete-profile/${userId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      })
  ...
  const handleUpdateSubmit = (e) => {
    ...
    axios.put(`http://localhost:5000/user/update/${userInfo._id}`, editForm, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })
